...
class SessionRepository:
    ...
    def clear_all_data(self) -> None:
        """Delete all activity records and reset their row identifiers."""

        with self.database.connect() as connection:
            connection.execute("DELETE FROM context_events")
            connection.execute("DELETE FROM breaks")
            connection.execute("DELETE FROM sessions")
            connection.execute(
                """
                DELETE FROM sqlite_sequence
                WHERE name IN ('context_events', 'breaks', 'sessions')
                """
            )
