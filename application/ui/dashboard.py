...
class DashboardApp:
    ...
    def start(self) -> None:
        ...
        self.root.after(500, self._offer_macos_permissions)

    def _offer_macos_permissions(self) -> None:
        ...
        if not messagebox.askyesno(
            "启用活动分析权限",
            (
                "AI Desk 需要“输入监控”和“辅助功能”权限，才能分析鼠标与键盘"
                "活动。\n\n不会保存鼠标坐标、移动轨迹、实际按键或输入内容。"
                "\n\n是否请求 macOS 权限？"
            ),
            parent=self.root,
        ):
            return
        updated = request_macos_permissions()
        if not updated.input_monitoring:
            open_macos_permission_settings("input")
        elif not updated.accessibility:
            open_macos_permission_settings("accessibility")
        messagebox.showinfo(
            "完成权限设置",
            (
                "请在 macOS“隐私与安全性”中启用 Terminal（或运行 AI Desk "
                "的应用）。macOS 不允许应用自动打开自己的权限开关。\n\n"
                "完成后请完全退出 Terminal，重新打开并启动 AI Desk。"
            ),
            parent=self.root,
        )
    ...
    def _build_summary(self, panel: tk.Frame) -> None:
        ...
        summary_body = tk.Frame(panel, bg=self.PANEL)
