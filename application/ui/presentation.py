...
def _timeline_row(item: dict[str, Any], local_timezone: tzinfo) -> TimelineRow:
    ...
    browser_title = str(payload.get("browser_title") or "").strip()
    browser_url = str(payload.get("browser_url") or "").strip()
    if browser_title or browser_url:
        try:
            hostname = urlparse(browser_url).hostname or ""
        except ValueError:
            hostname = ""
        tab_detail = browser_title or hostname or "浏览网页"
        if hostname and hostname.casefold() not in tab_detail.casefold():
            tab_detail = f"{tab_detail} · {hostname}"
        detail = f"{app} — {tab_detail}"
    else:
...
def format_summary(generation: SummaryGeneration) -> str:
    ...
    return f"{summary.headline}\n\n{summary.focus_assessment}".strip()
