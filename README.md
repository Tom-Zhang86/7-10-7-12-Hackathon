...
When Google Chrome is the frontmost application, the collector also records
the active tab title and URL so tab and website changes appear in the activity
timeline. macOS may request Automation permission for Terminal or Python to
control Google Chrome. Approve it under
`System Settings → Privacy & Security → Automation`. Capture remains limited
to the active tab while AI Desk is in the `Working` state.

## Privacy-preserving attention analysis

`application.activity` provides deterministic classifiers for aggregate mouse
activity, aggregate keyboard activity, visible-interface relevance, and likely
attention state. Keyboard classification accepts only counts, burst totals,
typing speed, and inactivity duration; actual keys are neither accepted nor
stored. The attention classifier requires at least five minutes of sustained
inactivity before returning `POSSIBLY_ZONED_OUT`, and sensor-confirmed absence
with no interaction takes precedence as `AWAY`.

While the state is `Working`, `ActivityMetricsCollector` writes one aggregate
`attention_window` record per minute. The daily-summary aggregator converts
those records into an attention-state duration breakdown and aggregate click,
scroll, and keypress counts. The AI and local fallback summaries may use that
breakdown as supporting evidence, but never receive actual keys or stored
pointer coordinates. On macOS, global input counting requires Input Monitoring
or Accessibility permission for the Terminal/Python process.
The dashboard checks both permissions at startup. With the user's consent it
triggers the official macOS permission requests and opens the relevant Privacy
& Security pane. macOS still requires the user to enable the switch manually;
an application cannot approve its own TCC permissions.

### Concentration-analysis rubric

Daily explanations use a context-aware temporal rubric rather than treating
activity as concentration. The evaluator distinguishes focused production,
focused reception/reasoning, focused task-supporting transitions, active
distraction, passive distraction, and away. It combines mouse structure,
keyboard rhythm, interface semantics, dwell and return sequences, change
origin, presence, and temporal persistence. Cognitive engagement is reported
separately from goal alignment, and uncertainty is retained. Current
probabilities are heuristic estimates; personal/task baselines and empirical
calibration require future labeled data.
