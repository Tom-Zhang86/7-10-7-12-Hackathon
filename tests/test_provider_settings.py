...
from application.providers.client import (
...
class ProviderClientTest(unittest.TestCase):
    def test_deepseek_json_output(self) -> None:
        calls = []
                ...
                "choices": [
                    {
                        "message": {
                            "role": "assistant",
                            "content": json.dumps(VALID_SUMMARY),
                        }
                    }
                ]
            }
        ...
        self.assertEqual(
            calls[0][2]["response_format"],
            {"type": "json_object"},
        )
        self.assertEqual(calls[0][2]["max_tokens"], 4000)
