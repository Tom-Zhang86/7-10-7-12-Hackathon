...
class MacOSContextProviderTest(unittest.TestCase):
    ...
    def test_captures_active_chrome_tab_title_and_url(self) -> None:
        provider = MacOSContextProvider(
            platform_name="Darwin",
            runner=lambda _command, _timeout: SimpleNamespace(
                returncode=0,
                stdout=(
                    "Google Chrome\x1fAI Desk - Google Chrome"
                    "\x1fAI Desk\x1fhttps://example.com/docs\n"
                ),
                stderr="",
            ),
        )
        ...
        self.assertEqual(context.browser_title, "AI Desk")
        self.assertEqual(context.browser_url, "https://example.com/docs")
...
class ContextCollectorTest(unittest.TestCase):
    ...
    def test_records_switches_between_chrome_tabs(self) -> None:
        api = FakeContextAPI()
        collector = ContextCollector(
            api,
            SequenceProvider(
        ...
        self.assertTrue(collector.capture_once(3))
        self.assertTrue(collector.capture_once(3))
        self.assertEqual(len(api.records), 2)
        self.assertEqual(api.records[-1]["source"], "chrome_active_tab")
        self.assertEqual(
            api.records[-1]["payload"]["browser_url"],
            "https://news.example.com/article",
        )
