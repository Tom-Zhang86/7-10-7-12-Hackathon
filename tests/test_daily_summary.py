...
class FakeAggregator:
    ...
    def build_today(self):
        ...
        return {
            ...
            "behavior_metrics": {
                "effective_focus_seconds": 180,
                "focus_rate_percent": 75,
                "average_concentration_seconds": 90,
                "longest_concentration_seconds": 120,
                "distraction_period_count": 1,
                "average_recovery_seconds": 30,
            },
            "attention_windows": [
                {
                    "mouse_state": "ACTIVE",
                    "keyboard_state": "ACTIVE_TYPING",
                    "switching_pattern": "RELATED_SWITCHING",
                    "mouse_active_ratio": 0.5,
                    "mouse_movement_rate_per_minute": 30,
                    "mouse_click_rate_per_minute": 4,
                    "mouse_scroll_rate_per_minute": 2,
                    "mouse_longest_idle_gap_seconds": 12,
                    "mouse_average_velocity_pixels_per_second": 400,
                    "mouse_trajectory_efficiency": 0.8,
                    "mouse_directional_entropy": 0.2,
                },
                {
                    "mouse_state": "LOW_ACTIVITY",
                    "keyboard_state": "OCCASIONAL_TYPING",
                    "switching_pattern": "STABLE",
                },
                {
                    "mouse_state": "ACTIVE",
                    "keyboard_state": "NO_TYPING",
                    "switching_pattern": "UNRELATED_SWITCHING",
                    "recent_interfaces": [
                        "Google Chrome | LinkedIn | https://linkedin.com",
                        "Python",
                    ],
                },
            ],
...
class DailySummaryModelTest(unittest.TestCase):
    ...
    def test_prompt_requires_ai_attention_analysis_and_complete_output(self) -> None:
        prompt = build_user_prompt(
            {
                "attention_windows": [
                    {
        ...
        self.assertIn("concentration_rubric_assessment", prompt)
        self.assertIn("不得改算", prompt)
        self.assertIn("概率最高", prompt)
        self.assertIn("不显示概率", prompt)
        self.assertIn("focus_assessment", prompt)
        self.assertIn('"keypress_count":45', prompt)
...
class ManualSummaryServiceTest(unittest.TestCase):
    ...
    def test_retries_then_uses_and_persists_fallback(self) -> None:
        ...
        self.assertEqual(generation.summary.data_quality_note, "")
        self.assertEqual(
            generation.summary.headline,
            "今日专注表现",
        )
        self.assertEqual(
    ...
    def test_user_facing_metrics_show_only_decisive_outcome(self) -> None:
        service = ManualSummaryService(
            FakeAggregator(),
            FakeLLM(result=VALID_SUMMARY),
            self.store,
        ...
        assessment = service.generate_today().summary.focus_assessment

        self.assertIn("已分类时间专注率：75%", assessment)
        self.assertIn("AI 判断：今天的主要状态是", assessment)
        self.assertIn("已分类时间专注率：75%", assessment)
        self.assertNotIn("结果原因", assessment)
        self.assertNotIn("AI 量表分析", assessment)
        self.assertNotIn("综合专注概率", assessment)
        self.assertNotIn("置信度", assessment)
        self.assertNotIn("鼠标：", assessment)
        self.assertNotIn("界面切换：", assessment)
        self.assertIn("鼠标统计", assessment)
        self.assertIn("平均活跃占比：", assessment)
        self.assertIn("平均轨迹效率：", assessment)
        self.assertIn("平均方向熵：", assessment)
        self.assertIn("键盘统计", assessment)
        self.assertIn("平均按键间隔：", assessment)
        self.assertIn("停顿分布：", assessment)
        self.assertIn("修正：", assessment)
        self.assertIn("快捷键：", assessment)
        self.assertIn("重复：", assessment)
        self.assertIn("界面统计", assessment)
        self.assertIn("平均语义相似度：", assessment)
        self.assertIn("平均停留稳定度：", assessment)
        self.assertIn("切换来源：", assessment)
        service.close()
